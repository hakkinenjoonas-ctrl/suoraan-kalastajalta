...
begin
  ...
  if lower(coalesce(old.buyer_email, '')) = public.current_user_email()
    ...
    if new.billed_at is distinct from old.billed_at and old.billing_status not in ('invoiced', 'paid') then
      raise exception 'Buyer cannot modify billed_at for this offer';
    end if;

    if new.paid_at is distinct from old.paid_at and new.billing_status <> 'paid' then
