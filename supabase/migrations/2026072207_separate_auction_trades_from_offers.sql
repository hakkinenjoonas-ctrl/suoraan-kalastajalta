-- Keep fixed-price offers and auction trades in their own application tabs.
alter table public.buyer_offers
add column if not exists sale_method text not null default 'fixed_price';

alter table public.buyer_offers
drop constraint if exists buyer_offers_sale_method_check;

alter table public.buyer_offers
add constraint buyer_offers_sale_method_check
check (sale_method in ('fixed_price', 'auction'));

-- Mark both existing and future auction-created trades explicitly.
