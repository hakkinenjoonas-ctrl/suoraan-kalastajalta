-- Reveal completed auction party details only to the seller/owner and winning buyer.
...
as $$
    ...
    select a.*,
      ...
      case when a.status = 'sold' and (a.seller_user_id = auth.uid() or public.is_owner()) then (
        select jsonb_build_object(
          ...
          'delivery_postcode', b.delivery_postcode,
      ...
      case when a.status = 'sold' and a.winning_buyer_id = (select buyer_id from me) then (
        select jsonb_build_object(
          ...
          'email', p.email,
          'phone', p.phone,
          'commercial_fishing_id', p.commercial_fishing_id
  ...
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'area', area, 'municipality', municipality, 'spot', spot, 'gear', gear, 'notes', notes, 'bid_count', bid_count,
