...
import { formatSpeciesForSale, getCrayfishSizeLabel, isCrayfishSpecies } from "../lib/species.js";
import { supabase } from "../lib/supabase.js";
import { styles } from "../lib/ui.js";
import { invokeEdgeFunctionAuthenticated } from "../services/edgeFunctions.js";
import { MunicipalitySelect } from "./FormInputs.jsx";
...
function newVariant(unitType, suggestedGrossPrice = null, species = "") {
  const suggestedPrice = priceInput(suggestedGrossPrice);
  return {
    id: crypto.randomUUID(),
    unitType,
    label: unitType === "package" ? "1 kg pakkaus" : unitType === "piece" ? getCrayfishSizeLabel(species) : "",
    packageSizeKg: unitType === "package" ? "1" : "",
    // Saaliin oletushinta on kilohinta, joten sitä ei saa kopioida ravun
    // kappalehinnaksi. Kalastaja antaa jokaiselle kokoluokalle oman hinnan.
...
function initialDraft(entry, defaultPickupLocation, defaultPickupMunicipality) {
  const netPrice = decimal(entry?.pricePerKg);
  const suggestedGrossPrice = netPrice && netPrice > 0 ? calculateGrossPrice(netPrice) : null;
  const unitType = isCrayfishSpecies(entry?.species) ? "piece" : "package";
  return {
    productName: formatSpeciesForSale(entry?.species) || "Kalaerä",
    description: "",
    pickupMunicipality: defaultPickupMunicipality || "",
    ...
    paymentMethods: [],
    unitType,
    variants: [newVariant(unitType, suggestedGrossPrice, entry?.species)],
    suggestedGrossPrice,
  };
}

export default function SavedCatchConsumerSaleDialog({ entry, profile, accessToken, defaultPickupLocation, defaultPickupMunicipality, publicAppBaseUrl, onClose, onPublished }) {
  const [draft, setDraft] = useState(() => initialDraft(entry, defaultPickupLocation, defaultPickupMunicipality));
  const [imageFile, setImageFile] = useState(null);
  const [imagePreviewUrl, setImagePreviewUrl] = useState("");
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");
  const [availableKilos, setAvailableKilos] = useState(null);
  ...
  useEffect(() => {
    ...
    supabase.rpc(functionName, { p_catch_entry_id: entry.id }).then(({ data, error: availabilityError }) => {
      if (!active) return;
      if (availabilityError) {
        if (isCrayfish) setAvailablePieces(0);
        else setAvailableKilos(0);
        setError(availabilityError.message || "Saaliserän jäljellä olevaa määrää ei voitu tarkistaa.");
      } else {
        if (isCrayfish) setAvailablePieces(Math.max(0, Number(data || 0)));
        else setAvailableKilos(Math.max(0, Number(data || 0)));
      }
      setAvailabilityLoading(false);
    });
    return () => { active = false; };
  ...
  const catchPieces = Number(entry?.count || 0);
  const remainingKilos = getSavedCatchUnallocatedKilos(availableKilos || 0, allocation);
  const remainingPieces = Math.max(0, Number(availablePieces || 0) - allocation.pieces);
  ...
  const selectUnitType = (unitType) => {
    setDraft((current) => ({ ...current, unitType, variants: [newVariant(unitType, current.suggestedGrossPrice, entry?.species)] }));
  };
  ...
  const publish = async () => {
    ...
    const variants = draft.variants.map((variant) => ({
      ...
      unit_price_including_vat: draft.unitType === "package" || draft.unitType === "piece" ? decimal(variant.unitPrice) : null,
    ...
    if (!pickupMunicipality) return setError("Valitse noutopaikan paikkakunta.");
    ...
    if (draft.unitType === "piece" && variants.some((variant) => !(variant.unit_price_including_vat > 0))) return setError("Täytä jokaiselle rapukokoluokalle kappalehinta.");
    ...
    if (draft.unitType === "piece") {
      if (!(availablePieces > 0)) return setError("Saaliserästä ei ole enää vapaita rapuja myyntiin.");
      if (allocation.pieces > availablePieces) return setError("Kuluttajamyyntiin varattu kappalemäärä ylittää saaliserän vapaan rapumäärän.");
    } else {
      if (!(availableKilos > 0)) return setError("Saaliserästä ei ole enää vapaata määrää myyntiin.");
      if (allocation.maximum > availableKilos + 0.001) return setError("Kuluttajamyyntiin varattu enimmäispaino ylittää saaliserän vapaan määrän.");
    }
    ...
    try {
      ...
      const { data: listingId, error: listingError } = await supabase.rpc("publish_consumer_listing", {
        ...
        p_municipality: pickupMunicipality,
